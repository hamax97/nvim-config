require("my.colors.theme")

-- THEMES INSTALLED:
-- Gruvbox: https://github.com/ellisonleao/gruvbox.nvim
--   Nice dark color.
-- Catppuccin: https://github.com/catppuccin/nvim
--   Nice light color. It's well maintained. 
--   Look at all the configs it has. Supports many plugins.
--   Also has themes for iTerm2: https://github.com/catppuccin/iterm/tree/main

-- TODOs:
-- Configure the line numbers to show more color on the selected line.
-- Custom highlight the current line.
-- use plugin: indent-blankline.nvim for
--   showing vertical lines of blocks
--   showing tabs, trailing spaces, eols
